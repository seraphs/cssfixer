...
let Reader = {
  ...
  updatePageAction: function(tab) {
    ...
    if (tab.readerActive) {
      this.pageAction.readerModeCallback = function() {
            sendMessageToJava({
              gecko: {
                type: "Reader:Click",
              }
      ...
      this.pageAction.id = NativeWindow.pageactions.add({
        ...
        let currentUrl = tab.browser.currentURI;
        let url = currentUrl.spec;
        if (url == "about:home" || url == "about:privatebrowsing") {
          this.pageAction.readerModeCallback = function() {
            sendMessageToJava({
              gecko: {
                type: "Qrcode:Click",
          ...
          let iconURL = "";
          let isPrivate =
            PrivateBrowsingUtils.isWindowPrivate(tab.browser.contentWindow);
          // tab.readerEnabled XOR isPrivate
          if (tab.readerEnabled != isPrivate) {
            iconURL = "drawable://reader";
          } else {
            iconURL = "drawable://reader_light";
          }
          this.pageAction.id = NativeWindow.pageactions.add({
            title: Strings.browser.GetStringFromName("readerMode.enter"),
            icon: iconURL,
